...
module CtrlUnit(
    ...
    parameter Imm_type_J = 3'b011; //UJ--JAL
    ...
    assign ALUSrc_A = JALR | JAL | AUIPC;  //1---rs1  0---PC

    //assign ALUSrc_B = TO_BE_FILLED;//控制第二个操作数来源于立即数还是寄存器等，该信号取决于指令的种类
    assign ALUSrc_B = I_valid | L_valid | S_valid | LUI | AUIPC; //1---imm  0---rs2  jalr和jal的imm都是要提前算掉的
    ...
    assign rs1use =  R_valid | I_valid | B_valid | L_valid | S_valid | JALR; //是否用到了rs1
    ...
    assign hazard_optype = {2{R_valid | I_valid | JAL | JALR | LUI | AUIPC}}& HAZARD_EX  | //写寄存器
