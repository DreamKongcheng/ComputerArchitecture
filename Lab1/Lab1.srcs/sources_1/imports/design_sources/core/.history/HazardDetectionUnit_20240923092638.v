...
module HazardDetectionUnit(
    ...
    assign A_MEM_Forward = rs1use_ID &  //本条指令有rs1
                        (rs1_ID == rd_MEM) &  //前一前条指令的rd等于本条指令的rs1
                        (|rs1_ID) &  //本条指令的rs1不是0
                        (|rs1_ID) &
                        ~(rs1_ID == rd_EXE) & (hazard_optype_EX == HAZARD_EX);       
    //assign B_MEM_Forward = TO_BE_FILLED;//这个空比较长，有五个判断条件同时成立时这个信号才会触发  
    assign B_MEM_Forward = rs2use_ID & 
                        (rs2_ID == rd_MEM) & 
                        (|rs2_ID) &
                        ~(rs2_ID == rd_EXE) & (hazard_optype_EX == HAZARD_EX);
